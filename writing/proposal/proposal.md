...
abstract: |
    Computational models have had considerable success in modeling the acquisition, comprehension, and production of language. However, almost all models either rely on cognitively unrealistic computations or fail to capture the complexity of true natural language. This may be because most modeling efforts take into account either the constraints imposed by the linguistic data or the constraints imposed by cognition--rarely both. In the proposed project, we will attempt to carefully consider both of these constraints while constructing a computational model using suitable component of previous models. Hopefully, this will result in a model that more fully and accurately captures human language.
---

# Introduction
What is it about humans that allow us--and only us--to acquire and use language? Comparing language to the communication systems of other animals, it is clear that language is quite unique. Even with extensive time and effort, researchers have been unable to train our nearest genetic relatives to use language as well as a four year old child [citation], despite the fact that they outperform young children on many other cognitive tasks [citation]. Just as language is unique among communication systems, it seems that humans are unique among animals with regard to linguistic ability.

There are clearly many external features of language that make it unique [@hockett68], but the cognitive abilities underlying these external features is are a matter of great debate. Which cognitive abilities one identifies as critical for language depends largely on which external features of language are deemed most important. Authors who focus on the social aspects of language point to humans' unique social reasoning abilities [@worden98; @malle02]. Other authors who focus on the use of symbols in language, point to our referential abilities [@deacon98; @bates79]. Theoretical linguists, who focus mostly on the structure of language, claim it is the ability to _combine_ symbols that truly differentiates humans from other animals [@hauser02; @chomsky10].
...
Most theoretical work puts more emphasis on just one question and its constraints. On one end of the spectrum <!-- TODO: make clear that this is an example! -->, transformational syntactic theories focus entirely on explaining the linguistic data, creating increasingly complex structures to explain them [citation], explicitly casting aside the constraint of cognitive abilities as mere issues of "performance" [citation]. This results in unwieldy representations that seem impossible to learn [citation]. On the other end, SRN models restrict themselves to neurally inspired representations [although see citation], but are only able to capture specific structures, often using toy corpora [citation citation citation]. This gap between models that take linguistic complexity seriously and models that take cognitive limitations seriously has resulted in a dearth of comprehensive psycholinguistic models.
...
- They are _unsupervised_, able to capture linguistic structure based solely on examples of valid utterances. __[note about semi-desirability of no supervision]__
...
Lack of supervision is critical to maintain psychological plausibility. Any model that requires parsed trees as input data [e.g. citation] cannot possibly describe the child's learning process. Meanwhile, generativity and scalability are essential attributes of a model that claims to capture the complexity of natural language. [Perhaps more here?]
...
If the model only preformed this one step, it would be equivalent to a bigram model. In reality, ADIOS detects patterns recursively, so the effective Markov order can be considerably higher than two. <!-- TODO: fix following --> and it computes probabilities in both forward and backward directions. Without going into the details, ADIOS can recognize that "bread and butter" occurs frequently, and that "dog" is very often preceded by "the". The use of backward transitional probabilities, (i.e. the probability of one word preceding another) is especially important because it allows the identification of patterns like "the dog", where "the" can be followed by many words, but "dog" can only be preceded by a few.

Once ADIOS has identified a significant pattern, it creates a new vertex representing the entire sequence of words. For every path that passed through the full sequence, ADIOS replaces the edge leading to the first word with one leading to the newly constructed vertex. Similarly, the edge out of the final word is replaced with one out of the new vertex. <!-- TODO: not removed, make this  --> Additionally, all the edges between the individual words are removed. This can be conceptualized as _explaining away_ the transitional probability between the individual words.

By constructing these patterns, ADIOS represents the sequential structure of language. A critical aspect of this process is that pattern identification can occur hierarchically on existing patterns. After identifying "hammer time" as pattern, ADIOS can later recognize "stop hammer time" as a pattern composed of "stop" and "hammer time". A pattern can thus be represented as a tree, or equivalently with brackets: [stop [hammer time]]. 

Any linguist reading this proposal will notice something important missing from the tree above, part of speech labels. ADIOS models this kind of categorical knowledge with _equivalence classes_. The intuitive observation is that similar words occur in similar contexts. If you ignore order and treat the context as a bag of words, you get _distributional semantic models_, such as Latent Semantic Analysis [@lund96]. By incorporating order, however, ADIOS is able to capture both syntactic and semantic patterns. Notably, ADIOS does not distinguish between the two.

In slightly more detail, the categorical generalization algorithm consists of searching the graph for paths that overlap for all but one vertex. Each of these vertices has thus been found to occur in one identical context. A new vertex is created ... __[I don't understand this part very well yet. Maybe leave out this paragraph?]__
...
U-MILA's top-down segmentation is similar to the full ADIOS algorithm, but applied to only the 50-300 most recently seen words. Like in ADIOS, the algorithm starts with a string of words and  looks for sequences that occur multiple times <!-- TODO: change. OR put in quotation -->. The memory window is a queue rather than a graph, so this amounts to comparing the most recent $n$ words to every other sequence of $n$ words in memory. If a match is found, it is added as a new vertex in the graph. Unlike in ADIOS, this does not result in any changes to the vertices that the new vertex is composed of. As in ADIOS, equivalence classes are created when a partially overlapping section is found.
